...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void _switchToPregnancy() async {
    ...
    final updated = profile.copyWith(pregnant: true, lastPeriod: lastPeriod);
  ...
  void _setPostpartum() async {
    final profile = widget.storage.profile;
    if (profile == null) return;
    await widget.storage.saveProfile(profile.copyWith(mode: 'postpartum', pregnant: false));
    if (mounted) {
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(builder: (_) => resolveHome(widget.storage.profile)),
        (route) => false,
  ...
  void _setRecovery() async {
    final profile = widget.storage.profile;
    if (profile == null) return;
    await widget.storage.saveProfile(profile.copyWith(mode: 'recovery', pregnant: false));
    if (mounted) {
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(builder: (_) => resolveHome(widget.storage.profile)),
        (route) => false,
  ...
  Widget build(BuildContext context) {
    final profile = widget.storage.profile;
    final showFertile = profile?.fertile ?? true;
    final isAlreadyPregnant = profile?.pregnant ?? false;
